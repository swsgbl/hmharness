...
(逐案例 pass/tokens)——聚合数回答不了"哪个案例翻转"。首份 per-case 报告
定位:双臂同挂全部 15 个对抗/围栏案例(**该 bench 模型对这类 exactness 无能为力,
与技能无关**);-1 净差=易案例运行间噪声(丢 train-regex-hex/train-x-6,得
train-exact-cn)。诚实结论:对抗集对当前模型是"不可教"区,技能可见的提升
只能在"可教"失败类上发生。

## [0.14.4] - 2026-09-15

**围栏约定落地**(day-54/55,杠杆验证→仪器→技能闭环):裸探测证明
模型在代码围栏内逐字保留字面量(prose 中则 prettify)。①bench matchCase 改为
围栏感知(fenceInner:断言围栏内内容,容忍围栏标记尾随空格)——测试 +2;②技能
草稿改为教围栏约定("fence first, paste verbatim");③+5 围栏对抗案例
(train=45)。**首个非零 diff**:control 29/45 vs treatment 28/45(-2.2%,p=0.83,
仍 needs-data)——双臂首次真实分化(方向为负,诚实记录)。

## [0.14.3] - 2026-09-15

**--cases 解析 off-by-one 修复**(day-51,飞轮第三仪器缺陷):
`slice(9)` 比 `--cases=`(8 字符)多切一位,`--cases=30` 解析成 `0`→回退 12——大 n
实验从未真正跑过请求的案例数。修=按字面长度切。30 案例 n 下重跑:
control 29/30 vs treatment 29/30(tokens 396→416 注入可见),p=1.0,诚实 needs-data;
训练集扩至 30(再 +20 确定性案例);双形态草稿规范化(drafts/<n>.md +
draft/<n>/SKILL.md);周度金丝雀五连 needs-data。
...
本身很弱时(如 25%),候选 25% 无回归也能晋升。修=绝对质量下限:候选 train
通过率必须 ≥ `evolution.minPassRate`(config.json 可配,默认 0.6),否则拒绝
并写明"对弱基线无回归不算够好";晋升理由带 `floor ≥N%` 供证据页审计。
测试 +2(弱基线 0.5 被地板拒绝/0.4 覆盖放行 0.75);全套 222+3 绿。
