...
for (let i = 0; i < count; i++) {
  ...
  else if (variant === 2) tasks.push(`Read G:/${workspace.split('/').pop()}/${ws}/entry/src/main/module.json5 with read_file, then reply ONLY the mainElement value.`);
  else tasks.push(`Read C:/Users/hongfu/.hmharness/bench/cases/${t} with read_file, then reply ONLY the first word of the first line.`);
}
