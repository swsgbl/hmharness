...
**60 天(纵深)**:
- [x] **模型感知上下文工程·前三项(2026-09-08 落地,107/107)**——现状诚实盘点:
      已有=检索式记忆(词法 bigram top-k 注入)+字符预算压缩(保护头尾)+会话
      恢复+记忆蒸馏;~~全部"一刀切",不感知模型~~:
      - [x] contextWindow 登记表(kernel/window.ts):显式配置 > 保守登记表
            (claude 200K/glm 131K/deepseek 128K/gemini 1M…)> 未知回退旧默认;
            ProviderConfig.contextWindow 显式覆盖;
      - [x] 预算随窗口自适应:预算=窗口 tokens×2.5 字符/token×50% 历史占比,
            双端 clamp(40K~1M);**128K 窗口恰好=旧默认 160K 字符,未知模型
            行为零变化**;loop 每轮按预算压缩;
