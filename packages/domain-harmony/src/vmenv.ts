...
export const QEMU_SDK_HDC = 'QEMU_DIR/sdk-5.0.2/14/toolchains/hdc.exe';

export const VM_DEFAULTS = {
  qemuDir: 'QEMU_DIR/KaihongOS',
...
export const vmwareVmsTool: Tool = {
  ...
  parameters: {
    ...
    properties: {
      ...
      vm: { type: 'string', description: 'VM name or absolute .vmx path (list under VM_DIR)' },
      guestCmd: { type: 'string', description: 'for guest-run: command line to run inside the guest (requires VMware Tools)' },
      sshKey: { type: 'string', description: 'for check: path to the SSH key (default ~/.ssh/VM_KEY)' },
    },
    required: [],
  },
  needsApproval(args) {
  ...
  async execute(args) {
    ...
    try {
      ...
      if (action === 'check') {
        ...
        const key = String(args?.sshKey ?? join(process.env.USERPROFILE ?? '', '.ssh', 'VM_KEY'));
        const vms: Array<{ name: string; host: string; user: string }> = [
          { name: 'Ubuntu 26.04', host: 'VM_IP_PLACEHOLDER', user: 'VM_USER' },
          { name: 'Kali 2026.2', host: 'VM_IP_PLACEHOLDER', user: 'kali' },
          { name: 'Win10 Fresh', host: 'VM_IP_PLACEHOLDER', user: 'VM_USER2' },
          { name: 'Win11 Fresh', host: 'VM_IP_PLACEHOLDER', user: 'VM_USER' },
          { name: 'Omarchy 4.0.4', host: 'VM_IP_PLACEHOLDER', user: 'VM_USER' },
        ];
