...
test('resume picker modal: loads rollouts from HMH_HOME, arrows + Enter pick, Esc closes', async () => {
  ...
  try {
    ...
    try {
      ...
      // settle: the picker finishes its async preload after the first frame -
      // firing the arrow in the same tick raced the row state (flake)
...
test('TUI owns the terminal: stray console output lands in the transcript, never on the raw screen', async () => {
  // user-reported on host + every VM: MCP attach notices sprayed over the
  // alt screen and froze the picture until a keypress repainted
  const h = await makeTui();
  try {
    const rawWrites: string[] = [];
    const realWrite = process.stdout.write.bind(process.stdout);
    process.stdout.write = ((c: string) => { rawWrites.push(String(c)); return true; }) as typeof process.stdout.write;
    try {
      console.log('[mcp] context7: 2 tools attached');
      console.error('undici: connection terminated');
    } finally { process.stdout.write = realWrite; }
    const p = h.rt.paletteProbe();
    assert.equal(p.transcript.includes('[mcp] context7: 2 tools attached'), true, 'log folded into transcript');
    assert.equal(p.transcript.includes('undici: connection terminated'), true, 'error folded into transcript');
    assert.equal(rawWrites.some((w) => w.includes('context7')), false, 'nothing leaked to the raw screen');
  } finally {
    h.restore();
    // restoration check: console works normally again after destroy
    let restored = false;
    const realWrite2 = process.stdout.write.bind(process.stdout);
    process.stdout.write = ((c: string) => { if (String(c).includes('restored-probe')) restored = true; return true; }) as typeof process.stdout.write;
    try { console.log('restored-probe'); } finally { process.stdout.write = realWrite2; }
    assert.equal(restored, true, 'console restored after destroy');
  }
});

test('long user input folds to two lines + count marker (full text archived by the driver)', async () => {
  const h = await makeTui();
  try {
    h.rt.addUser('这是一个超长的任务提示词。'.repeat(60)); // ~62 wrapped lines at test width
    const p = h.rt.paletteProbe();
    const tr = p.transcript.split('\n');
    // blank + 2 content rows + folded marker + blank = 5 lines, not 60+
    assert.ok(tr.length <= 6, 'folded to a handful of rows, got ' + tr.length);
    assert.equal(/还有 \d+ 行已折叠|more lines folded/.test(p.transcript), true, 'count marker shown');
  } finally { h.restore(); }
});
