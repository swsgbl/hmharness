...
export interface Strings {
  ...
  opsRadarLine: (n: number, time: string) => string;
  opsRadarNone: string;
  opsWatchingLabel: string;
  opsHint: string;
...
const zh: Strings = {
  ...
  opsRadarLine: (n, time) => `生态雷达:已扫描 ${n} 次,最近一次 ${time}`,
  opsRadarNone: '生态雷达:还没有扫描记录——输入 /ops scan 立即扫描上游',
  opsWatchingLabel: '监控来源:',
  ...
  cmdOpsBrief: '查看最近一次生态简报',
  opsNoBrief: '还没有简报——先运行 /ops scan 生成今日简报',
...
const en: Strings = {
  ...
  opsRadarLine: (n, time) => `ecosystem radar: ${n} scans, last ${time}`,
  opsRadarNone: 'ecosystem radar: no scans yet - run /ops scan to pull upstream now',
  opsWatchingLabel: 'watching:',
  ...
  cmdOpsBrief: 'read the latest ecosystem brief',
  opsNoBrief: 'no brief yet - run /ops scan to write today one first',
