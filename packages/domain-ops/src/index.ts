...
/** One watched upstream repo, human-labelled - UI layers print these,
 *  never the bare keys (oh-ace means nothing to a person at a terminal). */
export interface RadarSourceInfo { key: string; label: string; repo: string }

/** Human one-liner for the source list: 'OpenHarmony 文档 (openharmony/docs) · …' */
export function describeSources(list: RadarSourceInfo[] = DEFAULT_SOURCES): string {
  return list.map((s) => `${s.label} (${s.repo})`).join(' · ');
}

export async function opsStatus(home: string): Promise<{ scans: number; lastScan?: string; sources: RadarSourceInfo[] }> {
  ...
  return {
    ...
    sources: DEFAULT_SOURCES.map((s) => ({ key: s.key, label: s.label, repo: s.repo })),
  };
}
...
export { opsNotify, sendNotification, channelTools } from './channel.ts';
